...
export const validate: PluginValidateFn<any> = async (
  ...
  if (!all.includes(ext)) {
    throw new Error(
      `Plugin "urql-introspection" requires extension to be one of ${all
        .map(val => val.replace('.', ''))
        .join(', ')}!`,
    );
  }
  ...
  if (config.module === 'commonjs' && extensions.ts.includes(ext)) {
    throw new Error(
      `Plugin "urql-introspection" doesn't support commonjs modules combined with TypeScript!`,
    );
  }

  if (config.useTypeImports && !extensions.ts.includes(ext)) {
    throw new Error(
      `Plugin "urql-introspection" doesn't support useTypeImports modules not combined with TypeScript!`,
    );
  }
};
